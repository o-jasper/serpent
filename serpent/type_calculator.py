...
def convert_type(name):
    ...
    return name[name.find('_to_',3,) + 4:]
    ...
    if isinstance(ast, astnode):
        if ast.fun == 'set':
            ...
            if ast.fun in fun_types:  # Use the function to calculate the type.
                return astnode('the', [fun_types[ast.fun](map(type_of, input)),
...
def max_type(a, b):
    if a[:4] == 'uint' and b[:4] == 'uint':
        ...
        return 'int' + str(max(int(a[4:]) + int(b[4:])))
    elif a[:4] == 'float' and b[:4] == 'float':
        return 'float' + str(max(int(a[4:]) + int(b[4:])))
    elif a[:4] == 'float':
        return a
    elif b[:4] == 'float':
        return b
...
for el in ['+', '-', '*', '/']:
    fun_types['address'] = two_to_n(num_fun_type)
